        ...
        >
          <div className="mb-8 flex items-center justify-between">
            <Link to="/dashboard" className="flex items-center gap-3">
              <span className="grid h-9 w-9 place-items-center rounded-xl bg-guvi-orange font-display text-lg font-bold">G</span>
      ...
      <header className="sticky top-0 z-20 flex h-16 items-center justify-between border-b border-ink/10 bg-paper/90 px-4 backdrop-blur-md sm:px-6 lg:h-20 lg:px-8">
        ...
        <div className="flex items-center gap-2 sm:gap-4">
          ...
          <button className="relative rounded-lg p-2 text-ink/60 hover:bg-ink/5">
            <Bell size={20} />
            <span className="absolute right-1 top-1 h-2 w-2 rounded-full bg-guvi-orange" />
          </button>
          
          <div className="flex items-center gap-3 rounded-xl border border-ink/10 bg-white px-3 py-2">
            <div className="h-8 w-8 rounded-full bg-guvi-orange/10 text-guvi-orange flex items-center justify-center font-semibold text-sm">
...
export function PanelError({ message = 'This panel could not load.' }: { message?: string }) { return <div className="flex min-h-44 flex-col items-center justify-center gap-3 rounded-2xl border border-dashed border-coral/30 bg-guvi-orange/5 p-6 text-center"><CircleAlert className="text-guvi-orange" size={22} /><p className="text-sm text-ink/60">{message}</p></div> }
export function EmptyState({ clear }: { clear?: () => void }) { return <div className="flex flex-col items-center justify-center py-20 text-center"><div className="mb-4 grid h-14 w-14 place-items-center rounded-2xl bg-mint/40"><WalletCards size={25} /></div><h3 className="font-display text-lg font-bold">No refunds match these filters</h3><p className="mt-2 text-sm text-ink/50">Try widening the date range or clearing one of your filters.</p>{clear && <button onClick={clear} className="mt-5 text-sm font-semibold text-guvi-orange hover:underline">Clear filters</button>}</div> }
export function Button({ children, className = '', ...props }: React.ButtonHTMLAttributes<HTMLButtonElement>) { return <button className={`inline-flex items-center justify-center gap-2 rounded-xl bg-guvi-orange px-4 py-2.5 text-sm font-bold text-white shadow-sm transition hover:bg-guvi-orange/90 disabled:cursor-not-allowed disabled:opacity-50 ${className}`} {...props}>{children}</button> }
export function PageHeading({ eyebrow, title, action }: { eyebrow: string; title: string; action?: ReactNode }) { return <div className="mb-8 flex flex-col justify-between gap-4 sm:flex-row sm:items-end"><div><p className="mb-2 text-[11px] font-bold uppercase tracking-[.18em] text-guvi-orange">{eyebrow}</p><h1 className="font-display text-3xl font-bold tracking-tight sm:text-4xl">{title}</h1></div>{action}</div> }
export function SlaCountdown({ dueAt }: { dueAt?: string }) { if (!dueAt) return <span className="text-xs text-ink/35">No SLA set</span>; const overdue = new Date(dueAt) < new Date(); return <span className={`text-xs font-semibold ${overdue ? 'text-guvi-orange' : 'text-ink/55'}`}>{overdue ? 'SLA breached' : `Due ${new Date(dueAt).toLocaleDateString(undefined, { month: 'short', day: 'numeric' })}`}</span> }
