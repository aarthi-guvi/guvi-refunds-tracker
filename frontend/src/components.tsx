        ...
        >
          <div className="mb-8 flex items-center justify-between">
            <Link to="/dashboard" className="flex items-center gap-3">
              <span className="grid h-9 w-9 place-items-center rounded-xl bg-coral font-display text-lg font-bold">G</span>
      ...
      <header className="sticky top-0 z-20 flex h-16 items-center justify-between border-b border-ink/10 bg-paper/90 px-4 backdrop-blur-md sm:px-6 lg:h-20 lg:px-8">
        ...
        <div className="flex items-center gap-2 sm:gap-4">
          ...
          <div className="flex items-center gap-3 rounded-xl border border-ink/10 bg-white px-3 py-2">
            <div className="h-8 w-8 rounded-full bg-coral/10 text-coral flex items-center justify-center font-semibold text-sm">
...
export function KpiCard({ label, value, hint, accent = 'mint' }: { label: string; value: string; hint?: string; accent?: 'mint' | 'coral' | 'yellow' | 'blue' }) { return <div className="panel relative overflow-hidden p-5"><div className={`absolute left-0 top-0 h-1 w-full bg-${accent}`} /><p className="text-xs font-semibold uppercase tracking-[.13em] text-ink/45">{label}</p><p className="mt-4 font-display text-3xl font-bold tracking-tight">{value}</p>{hint && <p className="mt-2 text-xs text-ink/45">{hint}</p>}</div> }
export function Skeleton({ className = '' }: { className?: string }) { return <div className={`animate-pulse rounded-lg bg-ink/10 ${className}`} /> }
export function PanelError({ message = 'This panel could not load.' }: { message?: string }) { return <div className="flex min-h-44 flex-col items-center justify-center gap-3 rounded-2xl border border-dashed border-coral/30 bg-coral/5 p-6 text-center"><CircleAlert className="text-coral" size={22} /><p className="text-sm text-ink/60">{message}</p></div> }
...
export function Button({ children, className = '', ...props }: React.ButtonHTMLAttributes<HTMLButtonElement>) { return <button className={`inline-flex items-center justify-center gap-2 rounded-xl bg-coral px-4 py-2.5 text-sm font-bold text-white shadow-sm transition hover:bg-coral/90 disabled:cursor-not-allowed disabled:opacity-50 ${className}`} {...props}>{children}</button> }
export function PageHeading({ eyebrow, title, action }: { eyebrow: string; title: string; action?: ReactNode }) { return <div className="mb-8 flex flex-col justify-between gap-4 sm:flex-row sm:items-end"><div><p className="mb-2 text-[11px] font-bold uppercase tracking-[.18em] text-coral">{eyebrow}</p><h1 className="font-display text-3xl font-bold tracking-tight sm:text-4xl">{title}</h1></div>{action}</div> }
export function SlaCountdown({ dueAt }: { dueAt?: string }) { if (!dueAt) return <span className="text-xs text-ink/35">No SLA set</span>; const overdue = new Date(dueAt) < new Date(); return <span className={`text-xs font-semibold ${overdue ? 'text-coral' : 'text-ink/55'}`}>{overdue ? 'SLA breached' : `Due ${new Date(dueAt).toLocaleDateString(undefined, { month: 'short', day: 'numeric' })}`}</span> }
