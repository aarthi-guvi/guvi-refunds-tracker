...
export function KpiCard({ label, value, hint, accent = 'guvi-success', icon }: { label: string; value: string; hint?: string; accent?: 'guvi-success' | 'guvi-green' | 'guvi-lightGreen' | 'guvi-dark' | 'guvi-orange' | 'guvi-navy'; icon?: ReactNode }) { return <div className="panel relative overflow-hidden p-5"><div className={`absolute left-0 top-0 h-1 w-full bg-${accent}`} /><div className="flex items-start justify-between"><div className="flex-1"><p className="text-xs font-semibold uppercase tracking-[.13em] text-ink/45">{label}</p><p className="mt-4 font-display text-3xl font-bold tracking-tight">{value}</p>{hint && <p className="mt-2 text-xs text-ink/45">{hint}</p>}</div>{icon && <div className="ml-4 text-ink/20">{icon}</div>}</div></div> }
...
export function EmptyState({ clear, message = 'No refunds match these filters' }: { clear?: () => void; message?: string }) { return <div className="flex flex-col items-center justify-center py-20 text-center"><div className="mb-4 grid h-14 w-14 place-items-center rounded-2xl bg-mint/40"><WalletCards size={25} /></div><h3 className="font-display text-lg font-bold">{message}</h3><p className="mt-2 text-sm text-ink/50">Try widening the date range or clearing one of your filters.</p>{clear && <button onClick={clear} className="mt-5 text-sm font-semibold text-[#00A651] hover:underline">Clear filters</button>}</div> }
