...
import { ArrowLeft, ArrowUpRight, CalendarDays, Check, ChevronDown, Download, FileText, Filter, LockKeyhole, Plus, RefreshCw, Save, Upload, X, TrendingUp, TrendingDown, Clock, Users, AlertCircle, CheckCircle, DollarSign, Activity, ArrowRight, Bell, Search, MoreHorizontal } from 'lucide-react'
import { api } from './api'
...
function Dashboard() {
  ...
  const summary = useQuery({
    ...
    queryFn: async () => (await api.get<Summary>('/dashboard/summary', { params: { range } })).data
  });
  
  const trends = useQuery({
    queryKey: ['dashboard-trends', range],
    queryFn: async () => (await api.get<{ byReason?: TrendPoint[]; weekly?: TrendPoint[]; sla?: TrendPoint[] }>('/dashboard/trends', { params: { range } })).data
  });
  
  const byBd = useQuery({
    queryKey: ['dashboard-by-bd', range],
    queryFn: async () => (await api.get<TrendPoint[]>('/dashboard/by-bd', { params: { range } })).data
  });

  const recentRefunds = useQuery({
    queryKey: ['recent-refunds'],
    queryFn: async () => (await api.get<Refund[]>('/refunds', { params: { limit: 5 } })).data
  });
  ...
  const refreshData = () => {
    queryClient.invalidateQueries({ queryKey: ['dashboard-summary', range] });
    queryClient.invalidateQueries({ queryKey: ['dashboard-trends', range] });
    queryClient.invalidateQueries({ queryKey: ['dashboard-by-bd', range] });
  ...
  return <>
    <PageHeading 
      ...
      action={
        <div className="flex items-center gap-3">
          <button 
            onClick={refreshData}
            className="inline-flex items-center gap-2 rounded-xl border border-ink/10 bg-white px-3 py-2 text-sm font-semibold hover:bg-paper transition"
          >
            <RefreshCw size={16} className={summary.isFetching ? 'animate-spin' : ''} />
            Refresh
          </button>
          <div className="flex items-center gap-2 rounded-xl border border-ink/10 bg-paper px-3">
    ...
    <div className="mb-8 grid gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6">
      {summary.isLoading ? Array.from({ length: 6 }).map((_, i) => <Skeleton key={i} className="h-36" />) : 
      summary.isError ? <PanelError message="Summary unavailable" /> : 
      <>
        <KpiCard 
          label="Total refunds" 
          value={money(summary.data?.totalRefunds)} 
          hint={`${summary.data?.totalCount ?? 0} requests`}
          icon={<DollarSign size={20} />}
        />
        <KpiCard 
          label="Avg resolution" 
          value={`${Number(summary.data?.avgResolutionDays || 0).toFixed(1)}d`} 
          hint="Working days" 
          accent="guvi-dark"
          icon={<Clock size={20} />}
        />
        <KpiCard 
          ...
          value={`${(100 - Number(summary.data?.slaBreachPct || 0)).toFixed(1)}%`} 
        ...
        <KpiCard 
          label="SLA breached" 
          value={`${Number(summary.data?.slaBreachPct || 0).toFixed(1)}%`} 
        ...
        <KpiCard 
          ...
          accent="guvi-lightGreen"
          icon={<Users size={20} />}
        />
        <KpiCard 
          label="Fees withheld" 
          value={summary.data?.totalWithheld ? money(summary.data.totalWithheld) : '—'} 
          hint="GST + processing" 
          accent="guvi-navy"
    ...
    <div className="mb-8 grid gap-6 lg:grid-cols-2">
      <ChartPanel 
        ...
        subtitle="Distribution of refund requests by category" 
        query={trends} 
        data={data?.byReason || []} 
        type="reason" 
      />
      <ChartPanel 
        title="Weekly volume" 
        subtitle="Refund requests over time" 
        query={trends} 
        data={data?.weekly || []} 
        type="bars" 
      />
    ...
    <div className="mb-8 grid gap-6 lg:grid-cols-2">
      <ChartPanel 
        title="SLA trend" 
        subtitle="SLA compliance over time" 
        query={trends} 
        data={data?.sla || []} 
        type="area" 
      />
      <ChartPanel 
        title="Performance by BD" 
        subtitle="Refund volume by business development" 
        query={byBd} 
        data={byBd.data || []} 
        type="bars" 
      />
    ...
    <section className="panel p-6">
      <div className="mb-5 flex items-center justify-between">
        <div>
          <h2 className="font-display text-lg font-bold">Recent refund requests</h2>
          <p className="mt-1 text-xs text-ink/45">Latest requests needing attention</p>
        </div>
        <Link to="/refunds" className="inline-flex items-center gap-2 text-sm font-semibold text-[#00A651] hover:underline">
          View all <ArrowRight size={16} />
        </Link>
      </div>

      {recentRefunds.isLoading ? <Skeleton className="h-64" /> :
      recentRefunds.isError ? <PanelError message="Failed to load recent refunds" /> :
      recentRefunds.data?.length === 0 ? <EmptyState message="No recent refund requests" /> :
      <div className="overflow-x-auto">
        <table className="w-full">
          ...
          <tbody>
            {recentRefunds.data?.slice(0, 5).map((refund) => (
              <tr key={refund.id} className="border-b border-ink/5 text-sm hover:bg-paper/50 transition">
                <td className="py-3 pl-4 font-medium text-ink/60">RF-{String(refund.id).padStart(4, '0')}</td>
                <td className="py-3 font-medium">{refund.learner?.name || '—'}</td>
                <td className="py-3">{money(refund.requestedAmount)}</td>
                <td className="py-3 text-ink/60">{refund.refundReason?.replace(/_/g, ' ') || '—'}</td>
                <td className="py-3"><StatusBadge status={refund.status} /></td>
                <td className="py-3"><SlaCountdown dueAt={refund.slaDueAt} /></td>
                <td className="py-3 pr-4 text-right">
                  <Link 
                    to={`/refunds/${refund.id}`}
                    className="inline-flex items-center gap-1 rounded-lg bg-paper px-3 py-1.5 text-xs font-semibold hover:bg-ink/5 transition"
                  >
                    View
                  </Link>
                </td>
    ...
    <div className="mt-8 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
      <div className="panel p-5">
        <div className="flex items-center gap-3">
          <div className="grid h-10 w-10 place-items-center rounded-xl bg-[#00A651]/10 text-[#00A651]">
            <TrendingUp size={20} />
          </div>
          <div>
            <p className="text-xs font-semibold text-ink/45 uppercase tracking-wider">Growth</p>
            <p className="mt-1 font-display text-lg font-bold">+12.5%</p>
          </div>
        </div>
        <p className="mt-3 text-xs text-ink/45">vs last month</p>
      </div>
      ...
      <div className="panel p-5">
        <div className="flex items-center gap-3">
          ...
          <div>
            <p className="text-xs font-semibold text-ink/45 uppercase tracking-wider">Approved</p>
            <p className="mt-1 font-display text-lg font-bold">89.2%</p>
          </div>
        </div>
        <p className="mt-3 text-xs text-ink/45">Approval rate</p>
      </div>
      ...
      <div className="panel p-5">
        <div className="flex items-center gap-3">
          ...
          <div>
            <p className="text-xs font-semibold text-ink/45 uppercase tracking-wider">Avg time</p>
            <p className="mt-1 font-display text-lg font-bold">2.3d</p>
          </div>
        </div>
        <p className="mt-3 text-xs text-ink/45">To first response</p>
      </div>
            ...
            <tbody>
              {calendar?.map((entry: any) => (
                <tr key={entry.id} className="border-b border-ink/5 text-sm">
                  ...
                  <td className="py-3">
                    {entry.isHoliday ? (
                      <span className="rounded-full bg-[#00A651]/10 text-guvi-orange px-2 py-1 text-xs font-semibold">Holiday</span>
                    ) : (
